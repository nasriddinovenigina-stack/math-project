...
- **`build.js`** — reads `content.js` and generates the entire static site into `public/`: one HTML page per topic per language (`public/<slug>.html` and `public/ru/<slug>.html`), both `index.html` files, the standalone `privacy`/`contact` pages, `sitemap.xml`, and `robots.txt`. Run with `node build.js` (no npm install needed for this step). No emoji/icons are used anywhere on the site (removed 2026-09-11 for a cleaner, more editorial look) — don't reintroduce a `TOPIC_ICONS`-style map without being asked.
...
2. Add a generator function + `GENERATORS` map entry in `public/script.js`.
3. Run `node build.js` to regenerate `public/`.
4. Sanity-check: `node --check public/script.js`, and ideally stress-test new generators headlessly (load `script.js` in a Node `eval` with a stubbed `document`, call the generator N times, assert `checkAnswer(correctAnswerText)` is true for every problem — this caught a real bug once, see git history "Add 10 new topics" commit).
5. `git add -A && git commit -m "..." && git push origin master` — both Netlify sites deploy automatically (`mathpracticehub` immediately; `mathpractise` too, once/if its credits allow — see deployment section above).
6. Verify live: `curl -s https://mathpracticehub.netlify.app/<slug> | grep '<title>'` (this one always reflects the latest push; `mathpractise.netlify.app` may lag if it's credit-blocked).
7. There's also a `public/_redirects` file (Netlify redirects syntax, `from to status` per line) — it's hand-maintained, NOT generated by `build.js`, so it survives rebuilds untouched. Currently redirects the old `/multiplying-fractions` URL (from before Fractions absorbed it) to `/fractions`. Add a line here whenever a topic slug changes or gets merged/removed, so old links/bookmarks don't just 404.
...
- The site now has 55 topics (was 30 originally). Added since: rounding, prime-factorization, unit-rate, simple-interest, surface-area, angles, foil, distance-formula, trigonometry, long-division, divisibility-rules, comparing-fractions-decimals, fraction-decimal-percent, unit-conversion, multi-step-equations, direct-inverse-variation, transformations, compound-interest, data-graphs, counting-principle, proportions, box-plots, congruent-similar-figures, volume-cylinder-cone-sphere, factoring. "Multiplying & Dividing Fractions" was added as its own topic, then later merged into the "Fractions" topic (which now covers all 4 operations via `sections: [...]`) — see `public/_redirects`.
- **2026-09-15:** Added a homepage topic-search box (input field above the topic grid, filters `.topic-card`s by label live via `initTopicSearch()` in `public/script.js`, shows a "no results" message when nothing matches) plus 5 new topics (50 &rarr; 55): `proportions` (grade 6, cross-multiplication), `box-plots` (grade 7, IQR/five-number summary), `congruent-similar-figures` (grade 8, scale factor), `volume-cylinder-cone-sphere` (grade 8, curved-solid volume formulas, uses π &asymp; 3.14 like `circles`), `factoring` (grade 9, factoring trinomials, reverse of FOIL — answer format is a comma-separated pair like the `systems` topic). All 5 generators were stress-tested (100 rounds &times; 2 languages each, `checkAnswer(correctAnswerText)` verified true every time) before committing.
- **2026-09-11:** Added bilingual `privacy` and `contact` pages (linked from every page's footer), redesigned the homepage hero into a split-panel layout (bold gradient headline left, a 2x2 grid of clickable topic tiles right, circular ring stat badges) inspired by an agency-portfolio reference, and removed all emoji/icons site-wide for a cleaner editorial look. Around the same time, `mathpracticehub.netlify.app` got stuck and stopped picking up new pushes (three commits in a row didn't deploy) while `mathpractise.netlify.app` deployed each one normally — the opposite of its usual reliability; if this recurs, check the Netlify dashboard under `nasriddinovenigina@gmail.com` for a stuck/failed deploy rather than assuming hub is the safe fallback.
